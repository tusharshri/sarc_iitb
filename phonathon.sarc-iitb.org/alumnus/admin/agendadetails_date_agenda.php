<?php
	...
	$list = $DBConn->get_array ("SELECT * FROM alumnus_basicdetail NATURAL JOIN alumnus_contactdetail NATURAL JOIN allotment NATURAL JOIN alumnus_agendaconfirmation NATURAL JOIN agenda NATURAL JOIN (SELECT volunteer_ID, name FROM volunteer) AS v WHERE date_of_confirmation=? AND agenda=?", array($date, $agenda));
?>
<html>
	<head>
