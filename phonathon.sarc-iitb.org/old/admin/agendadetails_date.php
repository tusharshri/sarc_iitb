<?php
	...
	$list = $DBConn->get_array ("SELECT * FROM alumnus_basicdetail NATURAL JOIN alumnus_contactdetail NATURAL JOIN allotment NATURAL JOIN alumnus_agendaconfirmation NATURAL JOIN agenda NATURAL JOIN (SELECT volunteer_ID, name FROM volunteer) AS v WHERE date_of_confirmation=? ORDER BY agenda", array($date));
?>
<html>
	<head>
...
<?php
	foreach ($list as $alum) {
		...
		if ($agenda != $alum['agenda']) {
			if ($num != 0) {
?>
			<tr onclick="goto('agendadetails.php?date=<?php echo $date ?>&agenda=<?php echo $agenda ?>')">
				<th>Total</th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th><?php echo $num ?></th>
			</tr>
<?php
			}
?>
			<tr onclick="goto('agendadetails.php?date=<?php echo $date ?>&agenda=<?php echo $alum['agenda'] ?>')">
