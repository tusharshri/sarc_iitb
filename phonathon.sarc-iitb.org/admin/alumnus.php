...
<html>
  ...
  <body id="body">
    ...
    <div id="smenu">
      <ul>
        <li class="back" onClick="back()"><< Back to Alloted Alumni</li>
...
<?php
  ...
  else if($status == "Done and Locked"){
?>
        <li onClick="attempt()" class="locked" id="lock">Unlock the Alumnus Profile</li>
<?php
  }
  else if($status == "Not Alloted"){
?>
        <li onClick="document.location.href='allotment.php';" id="lock">Not Alloted</li>
<?php
  }
?>
      </ul>
...
?>
                ...
                <tr>
                  <td>Date of Birth</td>
                  <td><input type="date" name="dob" class="date" value="<?php if ($basicdetails['dob'] != "0000-00-00") echo $basicdetails['dob']; ?>" /></td>
                </tr>
<?php
  }
?>
...
?>
                ...
                <tr>
                  <td colspan="2">
                    <u>Phone Numbers</u>
                    <p>Time there is around 
                    <?php 
                                        /*$pre_time = getdate();
                    $pre_time = $pre_time['hours'] + (($pre_time['min'] / 60))/100;
                    echo (($time['time_diff']) + $pre_time)%24;*/
                                        $time_diff = $DBConn->get_array("SELECT time_diff FROM timezone WHERE country IN (SELECT chapter FROM alumnus_donation WHERE PID=? )",array($PID));
                                        $time_diff = intval($time_diff[0][time_diff]);
                                          $offset  = $time_diff;
...
?>
                ...
                <tr>
                  <td>City</td>
                  <td><input type="text" name="city" id="city" value="<?php echo $contactdetails['city'] ?>" /></td>
                </tr>
                <tr>
                  <td>Country</td>
                  <td><input type="text" name="country" id="country" value="<?php echo $contactdetails['country'] ?>" /></td>
                </tr>
                <tr>
                  <td colspan="2">
                    <u>Phone Numbers</u>
      ...
      $(document).ready(function(){
        ...
        var r=<?php echo "'".urlencode($contactdetails['city'])."'"; ?>;
        var s=<?php echo "'".$contactdetails['country']."'" ?>;
        var p=r+','+s;
          console.log(p);
          $.getJSON("http://www.worldweatheronline.com/feed/tz.ashx?key=740c6c04d9145820123001&format=json&callback=?",{
          q: p  
          },function(data) { 
          $('#time_there').html(data.data.time_zone[0].localtime);
...
?>
            <tr>
              <td>Time<small>(min)</small></td>
              <td>
                                <input id="callduration" type="text" name="timeofcall" value="<?php echo $otherdetails['time'] ?>" /><br/>
                                <input id="callstart" type="button" value="Start"/>
                                <input id="callstop" type="button" value="Stop"/>
                                <input id="callreset" type="button" value="Reset"/>
                            </td>
...
<?php
  if ($status == "Ongoing") {
?>
          <input type="button" onClick="SubmitMailInfo()" value="Submit" /><a href="send_mail.php">Send Mail</a>
                        <?php
  }
?>
          </div>
