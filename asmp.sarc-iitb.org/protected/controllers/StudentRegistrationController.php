...
class StudentRegistrationController extends Controller
{
    ...
    public function actionError()
  {
    ...
    private function isauthorized($studentId,$step){
        if(Yii::app()->user->isGuest){
            ...
            $mRegistration=StudentRegistration::model()->findByAttributes(array('studentId'=>Yii::app()->user->id,'phaseYear'=>'02_2012'));
      ...
      // validate user input and redirect to the previous page if valid
            ...
            if($model->validate() && $model->register()){
                ...
                if($user->save()){
                    ...
                    $registration=StudentRegistration::model()->findByAttributes(array("phaseYear"=>"02_2012",'studentId'=>$user->id));//echo"sa";print_r($registration);
                    if($registration!=null){
                        if($registration->step==6){
                            if(MentorshipConnection::model()->exists('studentId='.$user->id.' AND phaseYear=\'02_2012\'')){
  ...
  public function actionSop()
  {
        ...
        if($this->isauthorized($studentId,2)){
            //$mStudent=Student::model()->with(array('registrations',array('condition'=>array('phaseYear'=>'02_2012'))))->findByPk($studentId);
            $mRegistration=StudentRegistration::model()->findByAttributes(array('studentId'=>Yii::app()->user->id,'phaseYear'=>'02_2012'));
            $mResume = new Resume;
  ...
  public function actionPreference()
  {
        ...
        if($this->isauthorized($studentId,5)){
			...
			//$alumniList= Alumnus::model()->with(array('profDetails','profDetails.industry'))->findAll(array('condition'=>'t.id>1999'));
            $alumniList= Alumnus::model()->with('profDetails')->findAll(array('condition'=>'t.id>2999'));
