...
*/
	...
	public function accessRules()
	{
		return array(
			array(
				'allow',
				'actions'=>array('preference','login','submitPreference','mentees'),
				'users'=>array('*'),
			),
            array(
                'allow',
                'actions'=>array('updateBasic','updatePersonal','updateProf','updateEmail','addPhoneNumber','addEmailId','viewAllDetails'),
...
*/
	...
	public function actionPreference(){
        ...
        if($this->isauthorized($alumnusId)){
		    if($mAlumnus=Alumnus::model()->with(array(
                        'preferredStudents.student.department'=>array('condition'=>"preferredStudents.studentId NOT IN (SELECT studentId FROM MentorshipConnection WHERE phaseYear='02_2011')",'order'=>"preferredStudents.preferenceIndex "),
                        'preferredStudents.student.registrations'=>array('condition'=>'registrations.phaseYear=\'02_2011\''),
                        ))->findByPk($alumnusId)){
    ...
    public function actionMentees(){
        ...
        if($this->isauthorized($alumnusId)){
            $mAlumnus=Alumnus::model()->findByPk($alumnusId);
            if($mMentees=MentorshipConnection::model()->with('student.department')->findAllByAttributes(array('alumnusId'=>$alumnusId))){
