...
class StudentController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			/*array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','sop', 'view', 'create','register'),
				'users'=>array('*'),
			),
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('update', 'create', 'sop','sopUpdate','preference',  'view','done','sam', 'submitPreference','thankyou'),
				'users'=>array('@'),
			),
           ...
           // print_r($model->errors);
            if($valid){
                ...
                if($model->resume->saveAs(Yii::app()->basePath . '/../resume/' . $mstudent->rollNumber.".".$model->resume->extensionName)){
                    ...
                    $this->redirect('../StudentMentorshipPreference/create');
                }
            }
        }
    ...
    public function actionPreference(){
        ...
        $alumniList= Alumnus::model()->with('alumnusProfDetails')->findAll('prefLocked=2');
        ...
        foreach($alumniList as $alumnus){
            $alumniPref=($alumnus->alumnusMentorshipPreferences);
    ...
    public function actionMentorList(){	
        $alumniList= Alumnus::model()->with(array('profDetails'))->findAll(array('order'=>'departmentCode'));
        $alumniProfList= AlumnusProfDetail::model()->findAll();
        $finalList=$alumniList;
        $alumniWeight=array();	
        foreach($alumniList as $alumnus){
