 ...
 */
class AlumnusProfDetail extends ActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('alumnusId, designation, company', 'required'),
			array('alumnusId, industryId, pincode, countryId,jobNumber', 'numerical', 'integerOnly'=>true),
			array('city, state', 'length', 'max'=>32),
            array('designation, company','length','max'=>64),
            array('address','safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, alumnusId, designation, company, industryId,jobNumber, address, city, state, pincode, countryId, updatedAt', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function getIndustry(){
        $mIndustry=Industry::model()->findByPk($this->industryId);
