...
class AlumnusEmailId extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('alumnusId, emailId, comment, confirmation, updatedAt', 'required'),
			array('alumnusId, status', 'numerical', 'integerOnly'=>true),
			array('emailId', 'length', 'max'=>64),
			array('emailId', 'email'),
			array('type', 'length', 'max'=>7),
			array('type', 'in', 'range'=>array('primary','work','other')),
			array('confirmation', 'length', 'max'=>16),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, alumnusId, emailId, type, comment, status, confirmation, updatedAt', 'safe', 'on'=>'search'),
		);
	}
