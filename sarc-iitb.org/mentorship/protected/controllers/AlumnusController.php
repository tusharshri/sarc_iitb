...
*/
  ...
  public function actionPreference(){
        ...
        if($mAlumnus=Alumnus::model()->with(array(
                        'preferredStudents.student.department'=>array('condition'=>"preferredStudents.studentId NOT IN (SELECT studentId FROM MentorshipConnection WHERE phaseYear='02_2013')",'order'=>"preferredStudents.preferenceIndex "),
                        'preferredStudents.student.registrations'=>array('condition'=>'registrations.phaseYear=\'02_2013\''),
                        ))->findByPk($alumnusId)){
    ...
    public function actionMentees(){
        ...
        if($this->isauthorized($alumnusId)){
            ...
            if($mMentees=MentorshipConnection::model()->with(array(
                        'student.department',
                        'student.registrations'=>array('condition'=>'t.phaseYear=\'02_2013\''),))->findAllByAttributes(array('alumnusId'=>$alumnusId))){
