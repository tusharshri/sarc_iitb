  ...
  public function accessRules()
  {
    return array(/*
      ...
      array('allow',  // allow all users to perform MentorList action
        'actions'=>array('mentorList', 'mentorListVisitingCampus'),
        'users'=>array('*'),
      ),
      array('allow', // allow authenticated user to perform 'create' and 'update' actions
        'actions'=>array('mentor','home','mentorprofile','complaint','sendmessage','messagedetails'),
        'users'=>array('@'),
      ),/*
...
*/
    ...
    public function actionRegister()
  {
    ...
    public function actionMentorList(){
      ...
      public function actionMentorListVisitingCampus(){
        $alumniList= Alumnus::model()->with(array('profDetails','profDetails.industry'))->findAll(array('condition'=>'t.id IN(60,502,457,154,7,318,260,493,31,488)'));
        $alumniProfList= AlumnusProfDetail::model()->findAll();
        $finalList=$alumniList;
        $alumniWeight=array();  
        foreach($alumniList as $alumnus){
    ...
    public function actionThankyou(){
        ...
        public function actionMentor(){
            ...
            //print_r($mStudent);
            if($mMentor=MentorshipConnection::model()->with(array('alumnus','alumnus.personalDetails','alumnus.department','alumnus.emailIds','alumnus.phoneNumbers','alumnus.profDetails'))->findByAttributes(array('studentId'=>$studentId,'phaseYear'=>'02_2013'))){
    ...
    public function actionHome(){
      $studentId=Yii::app()->session['studentId'];
      if($mStudent=Student::model()->with(array(
                        'department',
                        'registrations'=>array('condition'=>'phaseYear=\'02_2013\''),))->findAllByAttributes(array('id'=>$studentId))){
             $mMsg=Student::model()->with(array('outbox','inbox'))->findByPk($studentId);
    ...
    public function actionMentorprofile(){
      $studentId=Yii::app()->session['studentId'];
            ...
            //print_r($mStudent);
            if($mMentor=MentorshipConnection::model()->with(array('alumnus','alumnus.personalDetails','alumnus.department','alumnus.emailIds','alumnus.phoneNumbers','alumnus.profDetails'))->findByAttributes(array('studentId'=>$studentId,'phaseYear'=>'02_2013'))){
                $this->render('mentorprofile',array(
                    'mStudent'=>$mStudent,
                    'mAlumnus'=>$mMentor->alumnus
                ));
            }
    ...
    public function actionSendmessage(){
      $studentId=Yii::app()->session['studentId'];
            if($mStudent=Student::model()->findByPk($studentId)){
            
            if($mAlumnus=MentorshipConnection::model()->with(array('alumnus'))->findAllByAttributes(array('studentId'=>$studentId,'phaseYear'=>'02_2013'))){
