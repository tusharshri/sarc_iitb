...
class Alumnus_attended extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('attended,program_for','type','type'=>'text'),
			array('year', 'length', 'max'=>10),	
			//array('year','type', 'type' => 'date', 'message' => '{attribute}: is not a date!', 'dateFormat' => 'yyyy-MM-dd'),
		);
	}
	...
	public function getNotnull($arr){
		...
		if($arr->attended==NULL && $arr->year==NULL && $arr->program_for=='other')
			return false;
		else 
			return true;
